...
void CClassView::setValueName()
{

	Data *pData=Data::shareData();


	HTREEITEM hRoot = m_wndClassView.InsertItem(_T("监控变量列表"), 0, 0);
	m_wndClassView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);

	for(int i=0;i<pData->data_num;i++)
	{
		CString str;
		HTREEITEM hClass = m_wndClassView.InsertItem(pData->pData_info[i].node_name, 1, 1, hRoot);
		
		//pData->pData_info[i].node_id
		str.Format(_T("%d"),pData->pData_info[i].node_id);
		m_wndClassView.InsertItem(str, 3, 3, hClass);

		str.Format(_T("数据长度：%d"),pData->pData_info[i].node_len);
		m_wndClassView.InsertItem(str, 3, 3, hClass);

		str.Format(_T("是否有符号：%d"),pData->pData_info[i].node_sigh);
		m_wndClassView.InsertItem(str, 3, 3, hClass);

		str.Format(_T("数据最大值：%d"),pData->pData_info[i].node_max);
		m_wndClassView.InsertItem(str, 3, 3, hClass);

		str.Format(_T("数据最小值：%d"),pData->pData_info[i].node_min);
		m_wndClassView.InsertItem(str, 3, 3, hClass);
